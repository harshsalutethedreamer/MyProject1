...
public class NumeroDialog extends DialogFragment {
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if(command.equals(OPEN_CHOICE)){
            view = mLayoutInflater.inflate(R.layout.dialog_layout2, null);
        }else{
            view = mLayoutInflater.inflate(R.layout.dialog_layout, null);
        }
        if (command.equals(DELETE_RECORD)) {
            ...
            builder.setTitle(R.string.take_photo)
                    .setItems(R.array.photo_type, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            if(which==0){
